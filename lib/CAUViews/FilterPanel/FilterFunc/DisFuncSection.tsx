...
interface IDisFuncSectionProps {
  viewState: ViewState;
  terria: Terria;
  backendPath: string;
  btnText: string;
...
export const DisFuncSection: FC<IDisFuncSectionProps> = observer(({ viewState, terria, backendPath, btnText }) => {
  ...
  const onSearchButtonClick = async() => {
    // 不连接后端时，使用内置测试数据调试功能
    // if (viewState.ArrDisData.length === ArrDataTest1.length) {
    //   viewState.ArrDisData = ArrDataTest2;
    // } else {
    //   viewState.ArrDisData = ArrDataTest1;
    // }

   // 时间条件
   const startDate = viewState.leftDate.replace(/-/g, '');
   const endDate = viewState.rightDate.replace(/-/g, '');
  ...
  return (
    <Box column>
      ...
      <Box gap>
        ...
        >
          <TextSpan medium>{btnText}</TextSpan>
        </FuncButton>
        <FuncButton
          primary
          fullWidth
        ...
        >
          <TextSpan medium>{t("下载数据")}</TextSpan>
        </FuncButton>
      </Box>
    </Box>
  );
