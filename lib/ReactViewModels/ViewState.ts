...
export default class ViewState {
  ...
  /**
   * 在此处添加新的全局变量模型
   * 将筛选面板所需要的行政区范围和时间范围在此定义
   * 状态定义：起始时间、结束时间、当前树数、省份树数、市树数、县树数、市数组、县数组、查询数组
   */
  @observable retainFilterPanel: boolean = true; // 通过“Filter”按钮访问的筛选面板 (改为true,使得失焦的时候仍然保持打开状态)


  @action
  setRetainFilterPanel(retain: boolean) {
    this.retainFilterPanel = retain;
  }

  // 数据集窗口的相关设置
  @observable typePanelIsVisible: boolean = false;
  @observable typeInfo: string = "点击按钮选择数据集";

  @action
  openDataSet() {
    this.typePanelIsVisible = true;
  }

  @action
  closeDataSet() {
    this.typePanelIsVisible = false;
  }
   ...
   * 重点在查询功能
   */
  // 点击地球获得的位置，存储在这个变量，传递给函数使用
  @observable dataBuilderShown: boolean = false;
  @observable queryPosition: Cartesian3 | undefined = undefined;
  @observable ArrData: Array<dataInfo> = [];
  @observable ArrDisData: Array<disDataInfo> = [];
  // 用于点查询的全局变量
  @observable pickPosition: {
    longitude?: number;
    latitude?: number;
  } = {longitude: undefined, latitude: undefined};
  // 用于矩形查询的全局变量
  @observable squarePosition: {
    left?: number;
    bottom?: number;
    right?: number;
    top?: number;
  } = {left: undefined, bottom: undefined, right: undefined, top: undefined};
  @observable polygonPosition: number[][] = [];
  // 存储行政区数组
  @observable districtArr: number[][][] = [];

  @observable leftDate: string = "2023-01-01";
  @observable rightDate: string = "2023-12-30";
  // 选择起始时间后，保存数据
  @action
  handleLeftDateChange = (value: string) => {
    this.leftDate = value;
  ...
  // 选择结束时间后，保存数据
  @action
  handleRightDateChange = (value: string) => {
  ...
  @action
  toggleDataBuilder = () => {
    this.dataBuilderShown = !this.dataBuilderShown;
  }

  // 下一次查询前，将之前的变量都清除掉
  searchRangeClear = () => {
    this.pickPosition.longitude = undefined;
    this.pickPosition.latitude = undefined;
    this.squarePosition.left = undefined;
    this.squarePosition.right = undefined;
    this.squarePosition.top = undefined;
    this.squarePosition.bottom = undefined;
