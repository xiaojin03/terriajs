...
export default function upsertSearchProviderFromJson(factory, terria, stratumName, json) {
    const errors = [];
    ...
    return new Result(model, TerriaError.combine(errors, `Error upserting search provider JSON: \`${applyTranslationIfExists(uniqueId, i18next)}\``));
}
function setDefaultTraits(model) {
    const terria = model.terria;
    runInAction(() => {
        model.setTrait(CommonStrata.defaults, "flightDurationSeconds", terria.searchBarModel.flightDurationSeconds);
        model.setTrait(CommonStrata.defaults, "minCharacters", terria.searchBarModel.minCharacters);
        model.setTrait(CommonStrata.defaults, "recommendedListLength", terria.searchBarModel.recommendedListLength);
    });
}
//# sourceMappingURL=upsertSearchProviderFromJson.js.map